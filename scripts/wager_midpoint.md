...
({method_b_date}). This is the check-in, not the verdict — the wager cannot
resolve NO before {deadline}.
...
the first entry-level model reaching it had a median of {median_lag} months over
{n_pairs} pairs — pairs produced by {n_matchers} catch-up releases, so treat
{n_matchers} as the sample size that moved.

**The gap to close.** {baseline} at {baseline_aa}; the best entry-level model
